...
public class JamQ extends PApplet {
...
public void setup() {
  ...
  surface.setSize(PApplet.parseInt(1920*.5f), PApplet.parseInt(1080*.5f));
  surface.setLocation(displayWidth/2-PApplet.parseInt(1920*.5f)/2, displayHeight/2-PApplet.parseInt(1080*.5f)/2);
  icon = new PImage();
...
float dSoundFade = 0.03f;
float dScreenFade = 0.08f;

public void toggleFadeIO() {
  ...
  if (fadeNutral) {
    ...
    if (fadeIO) {
      globalMovieVolume = globalMovieVolume < 0.002f ? 0 : lerp(globalMovieVolume, 0, dSoundFade*2);
      screenFadeOpac = 255*globalMovieVolume;
      return;
    } else {
      globalMovieVolume = globalMovieVolume > 0.998f ? 1 : lerp(globalMovieVolume, 1, dSoundFade/4);
      screenFadeOpac = 255*globalMovieVolume;
      return;
    }
  }
...
public void playMovie() {
  ...
  tint(255, PApplet.parseInt(screenFadeOpac));
  ...
  if (endingMovie.time() > endingMovie.duration() - 0.07f) {
    endingMovie.jump(0);
    endingMovie.pause();
    image(endingMovie, 0, 0, width, height);
  }
  if (recallMovie.time() > recallMovie.duration() - 0.07f) {
    recallMovie.jump(0);
    recallMovie.pause();
    image(recallMovie, 0, 0, width, height);
  }
...
public void judgeAllMovieIsPlaying() {
...
public void recordAllMovieTime() {
...
public void adjustScreen() {
  if (!toggleFullscreen) {
    ...
    if (isMouseDragging) {
      surface.setLocation(locX+=(mouseX-pmouseX)/1.5f, locY+=(mouseY-pmouseY)/1.5f);
...
public void mouseDragged() {
  if(mouseButton == LEFT && !toggleFullscreen){
    if(tcnt>0){
      surface.setLocation(PApplet.parseInt(absmouse.x-xorigin), PApplet.parseInt(absmouse.y-yorigin));
    }
  }
}
...
int tcnt = 0;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "JamQ" };
    if (passedArgs != null) {
